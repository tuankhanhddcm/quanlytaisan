<table class="table table_sp ">
    ...
    <tbody >
        @if (isset($taisan) && count($taisan) >0)
                ...
                @endphp
                    <td style="text-align: center"><input type="text" {{isset($phieukk)?'disabled':''}} onreset="if( {{$sl}} != {{$item->soluong}}){ $(this).css('border-color','red'); } "  style="width: 100px;text-align: center;outline: none;border-color:#cccc;" class="soluongkiemke soluong" name="soluongkiemke[]" data-id="{{$item->ma_ts}}" value="{{$sl}}"></td>
                    <td style="text-align: center">{{number_format($item->nguyengia)}}đ</td>
                    <td style="text-align: center;border-bottom: 1px solid rgba(0,0,0,.1)" id="soluong{{$item->ma_ts}}"><span id ='tieuhao'>{{$item->muc_tieuhao}}</span></td>
